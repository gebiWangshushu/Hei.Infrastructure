...
namespace DemoApi.Net6.Services
{
    public interface IUserService
    {
        ...
        [HeiHystrix(OnError = typeof(ArgumentException), FallBackMethod = nameof(MyFallback), EnableCircuitBreaker = true, ExceptionsAllowedBeforeBreaking = 3, MillisecondsOfBreak = 10 * 1000)]
        //所有异常都熔断
        //  [HeiHystrix(nameof(MyFallback), EnableCircuitBreaker = true, ExceptionsAllowedBeforeBreaking = 3, MillisecondsOfBreak = 20 * 1000)]
        Task CircuitBreaker();
        ...
        [HeiHystrix( OnError = typeof(ArgumentException), FallBackMethod =nameof(MyFallback), MaxRetryTimes = 1, RetryIntervalMilliseconds = 4 * 1000)] //只有ArgumentException异常才重试
        Task<string> Retry();
